        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Tax Declaration PDF preview</p>
                </div>
            </div>
...
const title = computed(() => tdNumber.value ? `TD ${tdNumber.value}` : 'Tax Declaration PDF');
const filename = computed(() => `TD-${tdNumber.value || route.params.id}.pdf`);
...
function goBack() {
    ...
    else router.push('/tax-declarations');
}
...
async function loadPdf() {
    ...
    try {
        const [metaRes, pdfRes] = await Promise.all([
            axios.get(`tax-declarations/${route.params.id}`).catch(() => null),
            axios.get(`tax-declarations/${route.params.id}/pdf`, { responseType: 'blob' }),
        ]);
        if (metaRes?.data?.td_number) tdNumber.value = metaRes.data.td_number;
        const blob = new Blob([pdfRes.data], { type: 'application/pdf' });
        pdfUrl.value = URL.createObjectURL(blob);
    } catch (err) {
        let message = 'Could not generate the PDF for this Tax Declaration.';
